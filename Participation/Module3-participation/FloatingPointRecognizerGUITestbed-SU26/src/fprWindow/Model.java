	...
	public static String checkForValidFoatingPointNumber(String input, boolean uV) {
		...
		while (running) {
			...
			case 0: 
				...
				// NEW LOGIC:
				// State 0 only accepts a numeric digit as the first character
				// this prevents numbers such as .75 from being accepted

				// a digit transitions the FSM from State 0 to State 1
				if (currentChar >= '0' && currentChar <= '9') {
					nextState = 1;
				...
				// any other first character causes the FSM to stop
				else {
					
					running = false;
				}

				// State 0 is not an accepting state
				finalState = false;
